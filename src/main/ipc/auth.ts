...
import { initLocalAuth, registerLocalUser, loginLocalUser } from '../services/localAuthService'
...
export function registerAuthHandlers() {
  ...
  ipcMain.handle(IPC_CHANNELS.AUTH_INIT_LOCAL, async () => {
    return initLocalAuth()
  })

  // Register local user
  ipcMain.handle(IPC_CHANNELS.AUTH_REGISTER_LOCAL, async (_event, data: { nome: string; usuario: string; senha: string }) => {
    return registerLocalUser(data.nome, data.usuario, data.senha)
  })

  // Login local user
  ipcMain.handle(IPC_CHANNELS.AUTH_LOGIN_LOCAL, async (_event, data: { usuario: string; senha: string }) => {
    return loginLocalUser(data.usuario, data.senha)
  })
}
