...
import { User, Lock, UserPlus, LogIn, Eye, EyeOff, Loader2, Code2, Users, Cloud, HardDrive } from 'lucide-react'
import logo from '../../assets/logo.png'

export default function LoginScreen() {
  const { 
    login, register, loginLocal, registerLocal, 
    initAuth, initAuthLocal, isAuthLoading, onlineUsers, startPing, stopPing 
  } = useAuthStore()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'register') {
      ...
      const result = authMode === 'cloud' 
        ? await register(nome, usuario, senha)
        : await registerLocal(nome, usuario, senha)
      ...
      const result = authMode === 'cloud'
        ...
        : await loginLocal(usuario, senha)
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          ...
          <div className="bg-nova-bg-secondary border border-nova-border rounded-xl p-6">
            ...
            {/* Auth Mode Selector */}
            <div className="mb-5">
              <label className="block text-[11px] text-nova-text-secondary mb-2 font-medium text-center">Tipo de autenticação</label>
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`flex items-center justify-center gap-2 py-2 px-3 rounded-lg text-xs font-medium transition-all ${
                    authMode === 'local'
                      ? 'bg-nova-accent/20 text-nova-accent border border-nova-accent/50'
                      : 'bg-nova-input-bg text-nova-text-muted border border-nova-input-border hover:border-nova-accent/30'
                  }`}
                >
                  <HardDrive size={14} />
                  Local
              ...
              <p className="text-[10px] text-nova-text-muted text-center mt-1.5">
                {authMode === 'local' 
            ...
            {/* Online users counter - only show for cloud mode */}
            {authMode === 'cloud' && (
