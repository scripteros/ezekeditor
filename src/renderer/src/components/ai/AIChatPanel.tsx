...
import { Send, Bot, User, StopCircle, Settings, Sparkles, Copy, Check, CheckCircle2, Trash2, Save, Plus, Paperclip, X, Image, File, TerminalSquare, Mic, Database, Play, Globe, Eye, EyeOff, Loader2, Monitor, Zap } from 'lucide-react'
      ...
      <div className="h-[35px] min-h-[35px] flex items-center justify-between px-3 bg-nova-bg-secondary border-b border-nova-border glass-panel">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className={`p-1 rounded transition-colors ${showSkills ? 'bg-nova-accent text-white' : 'text-nova-text-muted hover:text-nova-text hover:bg-nova-hover'}`}
            title="Skills da IA"
          >
            <Zap size={14} />
          </button>
      ...
      {showSkills && (
        <div className="absolute inset-0 z-30 flex items-start justify-center bg-nova-bg/70 p-4 backdrop-blur-sm">
          <div className="glass-panel flex max-h-full w-full max-w-[560px] flex-col overflow-hidden rounded-xl border border-nova-accent/25 shadow-2xl shadow-black/30">
            <div className="flex items-center justify-between border-b border-nova-border bg-nova-bg-secondary/95 px-4 py-3 flex-shrink-0">
              <div className="flex items-center gap-2">
                <Zap size={15} className="text-nova-accent" />
                <h2 className="text-xs font-bold uppercase tracking-wider text-nova-text">Skills da IA</h2>
              </div>
              <button
