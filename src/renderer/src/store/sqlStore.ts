    ...
    (set, get) => ({
      ...
      syncToServer: async () => {
        ...
        if (authMode === 'local') {
          ...
          if (api.localConfigSaveSQL) {
            await api.localConfigSaveSQL(String(user.id), {
              connections,
              redisServers,
              editorLayout: get().editorLayout,
              resultView: get().resultView,
            })
          }
        } else {
          // Save to MySQL server (cloud)
          if (api.userSaveConfig) {
            await api.userSaveConfig(user.id as number, 'db_connections', connections)
            await api.userSaveConfig(user.id as number, 'db_redis_servers', redisServers)
            await api.userSaveConfig(user.id as number, 'db_active_connection', activeConnectionId)
            await api.userSaveConfig(user.id as number, 'db_active_redis', activeRedisServerId)
          }
        }
      },
      ...
      loadFromServer: async () => {
        ...
        if (authMode === 'local') {
          ...
          if (api.localConfigLoadSQL) {
            const result = await api.localConfigLoadSQL(String(user.id))
            if (result.success && result.configs) {
              const c = result.configs
              if (c.connections) set({ connections: c.connections })
              if (c.redisServers) set({ redisServers: c.redisServers })
              if (c.editorLayout) set({ editorLayout: c.editorLayout })
              if (c.resultView) set({ resultView: c.resultView })
            }
          }
        } else {
          // Load from MySQL server (cloud)
          if (api.userLoadConfigs) {
            const result = await api.userLoadConfigs(user.id as number)
            if (result.success && result.configs) {
              const c = result.configs
