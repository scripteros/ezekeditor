...
const api = {
  ...
  authInitLocal: () =>
    ipcRenderer.invoke(IPC_CHANNELS.AUTH_INIT_LOCAL) as Promise<{ success: boolean; error?: string }>,

  authLoginLocal: (data: { usuario: string; senha: string }) =>
    ipcRenderer.invoke(IPC_CHANNELS.AUTH_LOGIN_LOCAL, data) as Promise<{ success: boolean; user?: { id: string; nome: string; usuario: string }; error?: string }>,

  authRegisterLocal: (data: { nome: string; usuario: string; senha: string }) =>
    ipcRenderer.invoke(IPC_CHANNELS.AUTH_REGISTER_LOCAL, data) as Promise<{ success: boolean; user?: { id: string; nome: string; usuario: string }; error?: string }>,
  ...
  // Local Config (JSON file for local auth)
  localConfigInit: () =>
    ipcRenderer.invoke(IPC_CHANNELS.LOCAL_CONFIG_INIT) as Promise<{ success: boolean; error?: string }>,

  localConfigSaveAI: (userId: string, data: {
    acquiredAPIs?: string[]
    enabledAIProviders?: string[]
    savedConfigs?: any[]
    activeConfig?: any
    ...
    ipcRenderer.invoke(IPC_CHANNELS.LOCAL_CONFIG_SAVE_AI, userId, data) as Promise<{ success: boolean; error?: string }>,

  localConfigLoadAI: (userId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.LOCAL_CONFIG_LOAD_AI, userId) as Promise<{ success: boolean; configs?: any; error?: string }>,

  localConfigSaveSQL: (userId: string, data: {
    connections?: any[]
    redisServers?: any[]
    editorLayout?: string
    resultView?: string
  }) =>
    ipcRenderer.invoke(IPC_CHANNELS.LOCAL_CONFIG_SAVE_SQL, userId, data) as Promise<{ success: boolean; error?: string }>,

  localConfigLoadSQL: (userId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.LOCAL_CONFIG_LOAD_SQL, userId) as Promise<{ success: boolean; configs?: any; error?: string }>,
  ...
  winprocListProcesses: () =>
    ipcRenderer.invoke('winproc:list-processes') as Promise<{ success: boolean; data?: any[]; error?: string }>,

  winprocListWindows: () =>
    ipcRenderer.invoke('winproc:list-windows') as Promise<{ success: boolean; data?: any[]; error?: string }>,

  winprocDumpMemoryStrings: (pid: number) =>
    ipcRenderer.invoke('winproc:dump-memory-strings', pid) as Promise<{ success: boolean; pid: number; strings?: string[]; count?: number; error?: string }>,

  winprocReadProcessMemory: (pid: number, address?: string, size?: number) =>
    ipcRenderer.invoke('winproc:read-process-memory', pid, address, size) as Promise<{ success: boolean; pid: number; hex?: string; error?: string }>,

  winprocGetUITree: (windowHandle: number) =>
    ipcRenderer.invoke('winproc:get-ui-tree', windowHandle) as Promise<{ success: boolean; handle: number; uiTree?: string[]; error?: string }>,
